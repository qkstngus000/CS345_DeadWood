...
public class CastingOffice extends Room {
	 ...
	 * Description:
	 * This just acts as a wrapper method for the private method, upgradeManager(),
	 * which actually handles players purchasing rank upgrades.
	 */
	 ...
	 * None
	 * Description:
	 * Function prints out rank upgrade information to user if user wants to do some
	 * action in casting office.
	 */
	private void printCastingInfo() {
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 3; j++) {
				if (j == 0)
					System.out.printf("%-4d", castingInfo[i][j]);
				else
					System.out.printf("|%-15d", castingInfo[i][j]);
			}
			System.out.println("\n");
		}
	}
	 ...
	 * Returns:
	 * bool: true if player purchased a rank, false if player backed out or had
	 * insufficient funds.
	 * Description:
	 * Handles what happens when a player attempts to purchase a rank. Takes input
	 * from player for which rank to buy
	 * and what currency to use, and makes the purchase by setting the appropriate
	 * values in the player if the player
	 * has enough resources to make the purchase, and if the player does not already
	 * possess an equal or higher rank.
	 */
	private boolean upgradeManager(Player p) {
		printCastingInfo();
		// Repeat until the user enters a valid input
		while (true) {
			// System.out.println("Enter the level of rank you'd like to buy, or 'q' to go
			// back:");
			// String usrEntry = DeadWood.feed.nextLine();

			ArrayList<String> options = new ArrayList<String>();
			for (int i = 0/* p.getRank()-1 */; i < castingInfo.length; i++) {
			...
			if (usrEntry.equals("Go Back")) {
				...
				// Commented out codes are for when we want our upgrade to be always higher than
				// the player rank, so that player cannot degrade their ranks
				for (int i = 0/* p.getRank()-1 */; i < castingInfo.length; i++) {
	...
	/**
	 * Attempt to purchase a rank for a player and display error messages if there
	...
	private boolean purchaseRank(Player p, int r) {
		...
		while (true) {
			// System.out.println("Enter 'm' to pay with money, 'c' to pay with credits, or
			// 'q' to go back:");
			ArrayList<String> options = new ArrayList<String>();
