...
*/
public class Board {
	private static int scenesLeft; // Number of rooms with currently active scenes
	 ...
	 * Description:
	 * Initializes the board as well as running XMLParseBoard() and XMLParseCard()
	 * to create the deck and the room list.
	 */
	public Board() {
	...
	public static void decrementScenesLeft() {
		scenesLeft--;
	}
	 ...
	 * Function: reset
	 * Description:
	 * calls distributeCard() on every SceneRoom in the board to repopulate the
	 * board with scenes.
	 ...
	 * Returns:
	 * bool: whether or not the operation was performed successfully. Will only fail
	 * if no cards
	 * are left in the deck.
	 * Description:
	 * Pops a random SceneCard from the deck and gives it to SceneRoom 's' by
	 * updating its scene field.
	 * The SceneCard is then added to the discard pile.
	 * The SceneCard given to s is a shallow copy of the one in the discard pile so
	 * as to avoid making
	...
	/*
	 * Function: newDeck
	 * Description: replaces cards from the discard list into the deck to reset it
	 * for a new game.
	 */
	public void newDeck() {
		deck.addAll(discard);
	...
	public static void notifyClosing() {
		decrementScenesLeft();
		System.out.println("Notified closing room to board                   " + scenesLeft);
	}
}
